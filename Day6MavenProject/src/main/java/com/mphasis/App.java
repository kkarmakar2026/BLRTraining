...
public class App {
    public static void main(String[] args) {
        ...
        Employee emp1 = (new Employee(101, "Koushik", 55000, "Developer", "IT"));
        Employee emp2=(new Employee(102, "Sham", 60000, "Tester", "QA"));
        Employee emp3=(new Employee(103, "Naresh", 75000, "Manager", "Sales"));
        Employee emp4 =(new Employee(104, "Ramesh", 50000, "Analyst", "Finance"));
        Employee emp5 =(new Employee(105, "Suresh", 80000, "Architect", "IT"));
        Employee emp6=(new Employee(106, "Balaji", 45000, "Support", "Customer Service"));
        
        
        ArrayList<Employee> empList = new ArrayList<Employee>(Arrays.asList(emp1,emp2,emp3,emp4,emp5,emp6));
        
        empList.stream().filter(emp->emp.getSalary()>50000).forEach(System.out::println);
        
        empList.stream()
        .filter(emp -> "Finance".equals(emp.getDesg())) // filter employees with designation Finance
        .sorted((e1, e2) -> e1.getEmpName().compareTo(e2.getEmpName())) // sort by name
        .map(Employee::getEmpName) // map to employee names
